package com.neurotec.samples.multibiometric.multimodal;
...
import com.neurotec.samples.multibiometric.*;
import com.neurotec.samples.multibiometric.preferences.MultimodalPreferences;
...
//		});
		...
		imageVoice.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				Intent voiceActivity = new Intent(MultiModalActivity.this, com.neurotec.samples.multibiometric.multimodal.VoiceActivity.class);
			...
			switch (requestCode) {
				case MODALITY_CODE_FACE: {
					if (data != null) {
						...
						byte[] template = b.getByteArray("face");
						NLTemplate nLTemplate = new NLTemplate(new NBuffer(template));
						if (nLTemplate != null) {
				...
				case MODALITY_CODE_IRIS: {
					if (data != null) {
						Bundle b = data.getExtras();
						byte[] template = b.getByteArray("iris");
						NETemplate nETemplate = new NETemplate(new NBuffer(template));

						if (nETemplate != null) {
							for (NERecord rec : nETemplate.getRecords()) {
								mIris.add(rec);
							}
						}
					}
				} break;
				case MODALITY_CODE_VOICE: {
					if (data != null) {
						Bundle b = data.getExtras();
						byte[] template = b.getByteArray("voice");
						NSTemplate nSTEmplate = new NSTemplate(new NBuffer(template));
						if (nSTEmplate != null) {
							for (NSRecord rec : nSTEmplate.getRecords()) {
								mVoice.add(rec);
							}
						}
					}
